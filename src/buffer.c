...
static int buffer_is_valid(const struct buffer *buf)
{
    ...
    if (buf->capacity == 0) {
        ...
        if (buf->read_index != 0 || buf->write_index != 0) return 0;
        return 1;
    }
    ...
    if (buf->read_index > buf->write_index) return 0;
    if (buf->write_index > buf->capacity) return 0;
    return 1;
}
...
{
    if (!buffer_is_valid(buf)) return -1;
    if (new_capacity == 0) return -1;
    if (new_capacity < buf->capacity) return -1;
    if (new_capacity > (size_t)BUFFER_MAX_CAPACITY) return -1;
...
int buffer_init(struct buffer *buf, size_t initial_size)
{
    ...
    if (initial_size == 0) initial_size = BUFFER_DEFAULT_SIZE;
    if (initial_size > (size_t)BUFFER_MAX_CAPACITY) return -1;
...
{
    if (!buffer_is_valid(buf)) return 0;
...
int buffer_ensure_writable_bytes(struct buffer *buf, size_t len)
{
    ...
    if (buf->write_index > (size_t)BUFFER_MAX_CAPACITY - len) return -1;

    /* 快路径：尾部可写空间足够 */
    if (buffer_writable_bytes(buf) >= len) return 0;
    ...
    if (new_capacity < required) return -1;
...
int buffer_append(struct buffer *buf, const void *data, size_t len)
{
    ...
    if (len != 0 && buf->write_index > (size_t)BUFFER_MAX_CAPACITY - len) return -1;

    if (buffer_ensure_writable_bytes(buf, len) != 0) return -1;
...
{
    if (!buffer_is_valid(buf)) return NULL;
